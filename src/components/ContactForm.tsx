...
const ContactForm: React.FC<ContactFormProps> = ({ isMainContactForm = false }) => {
  ...
  const validateForm = (formData: FormData) => {
    ...
    if (!formData.get('contact-first-name')) newErrors['contact-first-name'] = 'First name is required';
    if (!formData.get('contact-last-name')) newErrors['contact-last-name'] = 'Last name is required';
    if (!formData.get('organization-name')) newErrors['organization-name'] = 'Organization is required';

    const email = formData.get('contact-email') as string;
    if (!email) newErrors['contact-email'] = 'Email is required';
    else if (!/\S+@\S+\.\S+/.test(email)) newErrors['contact-email'] = 'Email is invalid';

    return newErrors;
  };

  const handleBlur = (field: FieldName) => {
    const ref = field === 'contact-message' ? messageRef.current : inputRefs[field].current;
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (Object.keys(newErrors).length > 0) {
      ...
      const ref = firstField === 'contact-message' ? messageRef.current : inputRefs[firstField]?.current;
      ref?.focus();
      return;
    }
    ...
    globalCount++;
    formData.append('_wpcf7_unit_tag', `wpcf7-f55-o${globalCount}`);
      ...
      <form onSubmit={handleSubmit} noValidate ref={formRef}>
        <div className="grid grid-cols-1 gap-x-8 gap-y-6">
          ...
          >
            ...
            <Button
              className="mt-5 w-full rounded-md border-2 border-[#0E8168] text-xl font-bold text-white uppercase bg-[#0E8168] px-6 py-3 text-center hover:bg-[#001d2f] hover:border-[#001d2f] hover:text-white"
              type="submit"
            >
              Submit
            </Button>
